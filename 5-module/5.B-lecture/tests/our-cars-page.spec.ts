...
test.describe('Our Cars Page', () => {
  ...
  test('Navigate to and Verify the "BMW M760" Details Page', async () => {
    const isCarVisible = await ourCarsPage.isCarButtonVisible(BMW_M760);
    expect.soft(isCarVisible).toBeTruthy();
    ...
    const carPriceTextVisible = ourCarsPage.getCarPriceText(BMW_M760, carPrice);
    expect.soft(carPriceTextVisible).toBeDefined();
    const bookNowButtonVisible = ourCarsPage.getBookNowButton(BMW_M760);
